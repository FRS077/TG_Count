    ...
    try:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"We couldn't find {file} in the specified URL.")
                quit()
            else:
...
while True:
    ...
    # Sort the callsign for every TG  
    ...
    final_tg = [key for value, key in count_lst[:5]]
    ...
    with Path(PATH_TO_WRITE, WRITE_FILE).open('+w', encoding='utf-8') as new:
        ...
        for tg_id in final_tg:
            # Check if the TG should be excluded  
            if tg_id in EXCLUDE_TG:
                continue  # Skip this TG if it's in the exclude list
    ...
    print(f"C'est fait !, maintenant nous allons attendre {TIME_TO_WRITE} minute(s)")
